...
# request
packet = create_packet(current_timestamp, LOCAL, OUT, SOURCE, DESTINATION, 0, 193, TER_ID)
...
packet = create_packet(current_timestamp, REMOTE, IN, SOURCE, DESTINATION, 0, 193, TER_ID)
df = pd.concat([df, pandas.DataFrame.from_records([packet])], ignore_index=True)
